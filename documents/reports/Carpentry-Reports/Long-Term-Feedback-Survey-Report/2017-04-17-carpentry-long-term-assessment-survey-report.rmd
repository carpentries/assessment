...
ggplot(data_workshops_tally, 
  ...
  ylab("% Respondents") +
  ggtitle("Majority of Respondents Attended One Carpentry Workshop") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic(base_size = 14)
```
...
ggplot(data_howlong_tally, 
  ...
  ylab("% Respondents") +
  ggtitle("Majority of Respondents Attended Workshop Over a Year Ago") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic(base_size = 14)
```
...
# get a few individual results to use in the text
Spreadsheets <- workshop_tools[workshop_tools$tool == 'Spreadsheets', ]$n
Cloud_Computing <- workshop_tools[workshop_tools$tool == 'Cloud Computing', ]$n
MATLAB <-  workshop_tools[workshop_tools$tool == 'MATLAB', ]$n
Mercurial <-  workshop_tools[workshop_tools$tool == 'Mercurial', ]$n
```
COME BACK TO THIS SECTION
From the table below we see that a large majority of respondents learned Git, Python, and Unix Shell. On the low end were Spreadsheets (`r Spreadsheets`), Cloud Computing (`r Cloud_Computing`), MATLAB (`r MATLAB`), and Mercurial (`r Mercurial`).
```{r include=FALSE}
...
```
The biggest pre-/post-workshop changes are a decline in the percentage of respondents that 'have not been using these tools' (`r -delta_pre_have_not_been_using`%), and an increase in the percentage of those who use the tools daily (`r delta_pre_daily_using`).
```{r}
pre_and_post_test <- 
pre_and_post_usage %>% 
  select(-perc) %>% 
...
A chi-square test indicates that there is a significant difference the frequencies of the pre- and post-workshop responses about programming use (chi-squared = `r round(unname(pre_and_post_test_result$statistic), 3)`, df = `r unname(pre_and_post_test_result$parameter)`, p-value = `r round(unname(pre_and_post_test_result$p.value), 5)`). The chi-squared standardized residuals for the post-workshop values show that significantly more respondents do programming daily after the workshop than expected if the workshop had no effect. Similarly, significantly fewer respondents program less that once per year after the workshop.

# Behaviors Adopted Post-Workshop
```{r}
# responses are in 'Behaviors-Adopted' through 'Column32'
# we use 'gather' to go from wide to long format
Behaviors <- 
data %>%
...
#- data management and project organization practices : Behavours-Adopted
...
```
```{r include=FALSE}
# These commands were used to get a count of the behaviors adopted.
...
We asked respondents to identify the behaviors they adopted as a result of completing a Carpentry workshop. We are happy to report that more than half of the respondents who answered this question (n= `r number_that_adopted_any_of_those_three`) have improved their data management and project organization practices, used programming languages for automation, and used version control to manage code. Additionally, respondents are *more confident* now than before they took a Carpentry workshop.
```{r}
kable(Behaviors, caption = "Behaviors Adopted by Respondents")
```
...
```{r}
# data are in 'How-Tools-Learned-Help' thtough 'Column37'
# we use 'gather' to go from wide to long format
...
data %>%
  ...
  arrange(desc(n)) %>% 
  rename(`Degree of improvement` = how_help)

kable(how_help,
      caption = "How the tools helped respondents")

```
```{r include=FALSE}
# I want to present these numbers as percentages, so I added up the count and divided by the number
# of respondents. I'm sure there's a better way to do this without doing so much math, so ideas are welcomed.
...
kable(Carpentry_Involvement,
      caption = "How respondents became involved")
```
```{r include=FALSE}
