...
```{r include=FALSE}
data <- read.csv("https://raw.githubusercontent.com/kariljordan/public-survey-info/master/documents/reports/Carpentry-Reports/Long-Term-Feedback-Survey-Report/2017-04-17-carpentry-long-term-assessment-survey-data-public.csv", na.strings = "")
```
...
# before it goes into the ggplot() function. Then you should be able to use reorder on that column 
# name in the ggplot() function
data_country_tally <-
data %>%
  group_by(Country) %>%
  tally() %>%
...
# covered in our workshops for individual.
tools_cols <-
  data %>%
    select(Content:Column22)
...
# The code segment below from Ben Marwick will get the tally of 
# combinations of tools and produce a table.
tool_combs <- 
...
kable(tool_combs_df[1:10, ])
```

Taking a look at which combinations of tools stood out in the data, we see from the matrix below that Git was frequently covered alongside Python, R, and/or the Unix Shell. Additionally, SQL was often covered with Git and/or the Unix Shell. This reveals that the majority of respondents have taken a Software Carpentry workshop.
```{r}
