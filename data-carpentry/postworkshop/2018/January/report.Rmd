...
__Data Collection Period__: 2017-Aug through 2018 Jan
...
library(srvyr)
```
This report and the figures created were create using Data Carpentry's post-workshop surveys collected August 7, 2017 through January 8, 2018. A [PDF](https://github.com/carpentries/assessment/blob/master/surveys/dc_postsurvey_INUSE.pdf) of the survey questions, the data used in this analysis, and full R code are located on the Carpentries [assessment](https://github.com/carpentries/assessment/tree/master/data-carpentry/postworkshop/2018/January) repo on GitHub. Feel free to use the data and [tell us](mailto: kariljordan@carpentries.org) about your findings.

```{r echo=FALSE}
data <- readr::read_csv("https://raw.githubusercontent.com/carpentries/assessment/master/data-carpentry/postworkshop/2018/January/data_180108.csv")
```
...
Learners were asked how likely they are to recommend this workshop to a friend or colleague using a sliding scale from zero to one hundred.
```{r include=FALSE}
data %>% 
  tally_and_perc(`Recommend`, na.rm = TRUE) %>% 
  kable()
```