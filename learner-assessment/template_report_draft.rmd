...
net_promoter_score_swc <- 
    ...
    count(category) %>%
    mutate("%" = (n / sum(n))*100)
    kable(net_promoter_score, format = 'markdown', row.names = NA, col.names = c ("Software Carpentry Promoter Score", "n", "%"))
...
For Software Carpentry respondents, `r percent_promoters_swc[3]`% are promoters.
```{r}
# Qualitative Analysis (SWC/DC)
```
...
=======
As the scoring for the above factors is ordinal from strongly disagree (1) to strongly agree (5), we show the mode (most frequent responses)
...
paired_data %>%
    ...
    ggplot() +
    geom_bar(aes(x = score, fill = factor(pre_post, levels = c("pre", "post"))),
             position = "dodge") +
    facet_wrap(~ skill) +
    scale_fill_viridis(
        name = "",
...
>>>>>>> 655915f3f2554b9ea21ec5233d4fa2f0d5683485
```{r, fig.height=16}
paired_data %>%
