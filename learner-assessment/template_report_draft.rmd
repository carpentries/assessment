...
ggassessment_lolli <- function(data, mapping) {
    ggplot(data, mapping) +
        ...
        geom_text(aes(label = n), size = 4, nudge_y = 1, hjust = 0) +
        coord_flip() +
        ylab("% Respondents") +
        theme_classic(base_size = 14)
}
...
dc_satisfaction %>%
    filter(!current_satisfaction %in% c("Not applicable",
                                        "Didn't answer")) %>% 
    ggassessment_hist(
        aes(x = factor(current_satisfaction,
    ...
    labs(
        ...
        caption = "'Not applicable' and 'Didn't answer' not shown"
    ) 
```
...
In terms of current programming usage, `r dc_percent_programming_rare`% of learners either never use programming, or use programming less than once per year, but no more than several times per year. Only `r dc_percent_programming_daily`% program on a daily basis. This is no surprise, as Data Carpentry workshops tend to attract novices.
...
ggknowledge <- function(data, mapping) {
    data <- mutate(data, text_pos = percent + 3)
    ggplot(data, mapping) +
        geom_col(position = "dodge") +
        geom_text(aes(label = n, y = text_pos), size= 4, hjust = 0, 
