...
dc_satisfaction <- dcpredata %>%
    select(current_satisfaction) %>%
    mutate(current_satisfaction = replace(current_satisfaction,
                                          is.na(current_satisfaction),
                                          "Didn't answer")) %>% 
    count(current_satisfaction) %>%
    mutate(percent = round(n/sum(n) * 100, 1)) %>% 
    arrange(factor(current_satisfaction, levels = satisfaction_levels))
