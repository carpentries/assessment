...
dcpredata <- readr::read_csv("data/180118_dcpre.csv")

# Data Carpentry Post-Workshop Data
dcpostdata <- readr::read_csv("data/180108_dcpost.csv")
```
...
## NPS
```
This is an overview of the number of learners attending
Data Carpentry and Software Carpentry workshops. `r n_learners_pre_dc` learners have attended Data Carpentry workshops, and `r n_learners_pre_swc` have attended Software Carpentry workshops. 
...
paired_data %>%
    ...
    geom_point(position = position_jitter(height = .1, width = .1), alpha = .2) +
    facet_wrap(~ skill) +
    geom_line(aes(group = UniqueID), alpha = .2)


    ggplot(aes(x = score, y = type)) +
    geom_point(aes(color = UniqueID)) + #, position = position_jitter(height = .1)) +
    geom_line(aes(color = UniqueID, group = UniqueID)) + #, position = "jitter") +
