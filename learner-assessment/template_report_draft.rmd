...
### Authors: [Kari L. Jordan](https://twitter.com/drkariljordan), [François Michonneau](https://twitter.com/fmic_)
### Published: July 2018
...
Since `r format(min(c(dc_dates, swc_dates)), format="%B %d, %Y")`, Software and Data Carpentry have collected information on learner demographics, perception of tools and confidence in working with data. As we continue in our goal to streamline processes as The Carpentries, the Assessment Team completed an analysis of the pre- and post-workshop surveys for both Software and Data Carpentry. The goal of this analysis is to understand the impact our workshops are having on learners, and how we can improve our surveys and assessment infrastructure. This report covers the workshops from `r format(min(swc_dates), format="%B %d, %Y")` to `r format(max(swc_dates), format="%B %d, %Y")` for Software Carpentry, and from  `r format(min(dc_dates), format="%B %d, %Y")` to `r format(max(dc_dates), format="%B %d, %Y")` for Data Carpentry.
...
Currently, Data and Software Carpentry use different surveys. In the coming months we plan to develop one common survey to be used for both Data and Software Carpentry.
...
Compared to Data Carpentry's learners, Software Carpentry's tend to have more experience with the tools covered in the workshops, and learners come to learn new and/or additional topics (`r p_swc_why_attending`%). It's also interesting to note that `r p_first_timers`% of Software Carpentry respondents are first-time attendees.
...
#dc_satisfaction %>%
#    render_table(current_satisfaction, 
...
dc_satisfaction %>%
    filter(!current_satisfaction %in% c("Not applicable",
                                        "Didn't answer")) %>% 
    ggassessment_lolli(
    ...
    labs(
        ...
        subtitle = "'Not applicable' and 'Didn't answer' not shown"
    ) 
```
...
dc_programming_usage %>%
    filter(!current_programming %in% c("Not applicable",
                                        "Didn't answer")) %>% 
    ggassessment_lolli(
...
In terms of current programming usage, `r dc_percent_programming_rare`% of learners either never use programming, or use programming less than once per year, but no more than several times per year. Only `r dc_percent_programming_daily`% program on a daily basis. This is no surprise, as Data Carpentry workshops are intended for novices.
...
Data Carpentry and Software Carpentry workshop participants often find out about our workshops through institution mailing lists (`r dc_exposure_mailing`% and `r swc_exposure_mailing`% respectively). However, "word-of-mouth" also plays a significant role in populating the workshops.
...
ggassessment_lolli(swc_comfortable, 
                   aes(y = factor(atmosphere, levels = rev(atmosphere_levels)),
                       x = percent),
                   color = "swc") +
    labs(
...
We want to be proactive in ensuring learners have access to whatever they need to participate in a workshop. Both Data and Software Carpentry learners are asked to inform workshop organizers if there is anything they need that would make their workshop experience better. Data Carpentry's respondents were asked if they had accessibility issues, and `r dc_accessibility_percent`% reported they did. After reading the open-ended responses, we see that the issues were related to not being able to hear and/or see in the back of the room. The Instructor Training Team has been made aware of this, and will be making recommendations.
...
The scoring for the above factors is ordinal from strongly disagree (1) to strongly agree (5).
...
```{r dc-paired-distribution-comparison, fig.height=12}
# 
# paired_data %>%
#     select(unique_id, skill,
#            pre = pre_feeling_score, post = post_feeling_score) %>%
#     filter(!is.na(pre) & !is.na(post)) %>%
#     gather(pre_post, score, -unique_id, -skill) %>%
...
#     complete(skill, pre_post, score,
#              fill = list(n = 0, n_total = 0, percent = 0 )) %>% 
#     ggplot(aes(x = score, y = percent/100, group = pre_post)) +
#     geom_col(aes(fill = pre_post),
#              position = "dodge") +
#     geom_text(aes(label = n, y = percent/100 + .03),
...
```{r dc-paired-distribution-shift, fig.height=12}
# 
# paired_data %>%
...
swc_ability_pre_plot <- swc_ability_pre %>%
...
swc_confidence_post_plot <- swc_confidence_post %>%
...
The Carpentries is a global community that has recognized the importance of bringing people to data through high-impact trainings. Thought the majority of Data Carpentry respondents attended a workshop in the United States of America (`r get_dc_country_percent("United States of America")`%), we see an increase in workshops in places like Ethiopia (`r get_dc_country_percent("Ethiopia")`%), Switzerland (`r get_dc_country_percent("Switzerland")`%), and India (`r get_dc_country_percent("India")`%).   
...
dc_status %>%
    render_table(
        question, 
        col.names = c("Data Carpentry's Respondents by Position", "n", "%")
    )
```
...
ggassessment_lolli(dc_status,
                   aes(y = factor(question, levels = rev(dc_status_levels)),
                       x = percent), color = "dc") +
    labs(
        title = "Data Carpentry Respondents by Position/Status"
    )
```
...
ggassessment_lolli(swc_status,
    ...
    labs(
        title = "Software Carpentry Respondents by Position/Status"
    )
```
...
In our workshops, we recommend that learners use their own machines. It is important for learners to leave the workshop with their own machine set up to do real work. Our instructors teach on three major platforms: UNIX/Linux, Mac OS X, and Windows. We see a very close representation of Windows (`r dc_os_win`%)  and Apple/Mac OS (`r dc_os_mac`%) users in our Data Carpentry workshops, and even a few UNIX/Linux users (`r dc_os_lin`%).
