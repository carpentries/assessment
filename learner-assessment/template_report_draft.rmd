...
```{r dc-paired-distribution-comparison}
...
paired_data %>%
    ...
    geom_col(aes(fill = pre_post),
             position = "dodge") +
    geom_text(aes(label = n, y = percent + 3),
...
```{r dc-paired-distribution-shift, fig.height=16}
...
```{r dc-tools-perception, fig.height=16}
# Data Carpentry pre-workshop neutral centered graph code
perception_levels <- c("Strongly disagree", "Disagree", "Neutral", "Agree",
                       "Strongly agree")

prep_likert_data <- function(.data, var_levels) {
    .data %>% 
        filter(has_answers(.)) %>%
...
prep_perception_tools <- . %>%
    ...
    prep_likert_data(perception_levels)
...
swc_pre_knowledge %>%
    prep_likert_data(pre_knowledge_levels) %>% 
    plot() %>%
    labs(
...
```{r swc-knowledge-change, fig.height=16}
...
ggknowledge <- function(data, mapping) {
    ggplot(data, mapping) +
        ...
        geom_text(aes(label = n, y = percent + 1), size= 4,
...
swc_pre_knowledge_plot <- swc_pre_knowledge %>%
    ...
    ggknowledge(aes(x = skill, y = percent,
                    fill = factor(level, levels = pre_knowledge_levels))) +
    labs(
        title =  "Knowledge Before Workshop"
    )

swc_post_knowledge_plot <- swc_post_knowledge %>% 
    prep_knowledge_plot(post_knowledge_levels) %>%
    ggknowledge(aes(x = skill, y = percent,
                    fill = factor(level, levels = post_knowledge_levels))) +
    labs(
        title = "Knowledge After Workshop"
    )
...
swc_ability_pre_plot <- swc_ability_pre %>%
    ...
    ggknowledge(aes(x = skill, y = percent,
                    fill = factor(level, levels = swc_ability_levels)))

swc_confidence_post_plot <- swc_confidence_post %>%
    prep_knowledge_plot(swc_confidence_levels) %>%
    ggknowledge(aes(x = skill, y = percent,
                    fill = factor(level, levels = swc_confidence_levels)))
...
get_percentage <- function(.data, .var, .value) {
    col_var <- enquo(.var)
    .data %>%
...
get_dc_country_percent <- function(country_var, .data = dc_country) {
    get_percentage(.data, .var = country, .value = country_var)
}
...
```{r dc-country-workshop-plot}
# Plot Data Carpentry's respondents by Country
ggplot(dc_country,
       aes(x = reorder(country, percent),
           y = percent)) +
    geom_col(fill = "darkcyan") +
    xlab("") +
    ylab("% Respondents") +
    coord_flip() +
    ggtitle("Breakdown of Data Carpentry's Survey Respondents by Country") +
...
dc_discipline_data <- dcpredata %>%
    select(starts_with("discipline"),
           - discipline_other) %>%
    filter(has_answers(.))

dc_discipline_keys <- dc_discipline_data %>%
    get_key_multi_answer

dc_discipline <- dc_discipline_data %>%
    tally_percent_multi_answer %>%
    inner_join(dc_discipline_keys, by = "column_name") 

get_dc_discipline_percent <- function(discipline_var, .data = dc_discipline) {
    get_percentage(.data, .var = column_name, .value = discipline_var) 
}
...
As previously mentioned, Data Carpentry's curricula is domain specific to Ecology, Genomics, Geospatial, and the Social Sciences. We see this in the distribution of respondents by discipline. `r get_dc_discipline_percent("discipline_10")`% are in the Life Science, while `r get_dc_discipline_percent("discipline_1")`%, `r get_dc_disciplie_percent("discipline_2")`%, and `r get_dc_discipline_percent("discipline_3)`% are in Agricultural or Environmental Sciences, Bioinformatics/Genomics, and Biomedical/Health Sciences, respectively.
...
swc_discipline_keys <- swc_discipline_data %>%
    get_key_multi_answer

swc_discipline <- swc_discipline_data %>%
    tally_percent_multi_answer %>%
    inner_join(swc_discipline_keys, by = "column_name")
...
dc_status_keys <- dc_status_data %>%
    get_key_multi_answer

dc_status <- dc_status_data %>%
    tally_percent_multi_answer %>%
    inner_join(dc_status_keys, by = "column_name")

get_dc_status_percent <- function(status_var, .data = dc_status) {
    get_percentage(.data, .var = column_name, .value = status_var)
}
...
swc_status <- swcpredata %>%
    single_answer_summary(status, swc_status_levels)

get_swc_status_percent <- function(status_var, .data = swc_status) {
    get_percentage(.data, .var = status, .value = status_var)
}
```

As many of The Carpentries' workshops are hosted on college campuses and other research-based communities, there is no surprise that the majority of respondents are Graduate Students (`r get_dc_status("status_2")`% - DC, `r get_swc_status_percent("Graduate Student")`% - SWC), Research Staff (`r get_dc_status("status_7")`% - DC,`r get_swc_status_percent("Research staff (including research programmer)")`% - SWC), and Postdoctoral Researchers (`r get_dc_status("status_9")% - DC, `r get_swc_status("Post-doctoral researcher")`% - SWC).

```{r dc-status-plot}
## Data Carpentry plot of learners by position/status
ggassessment_hist(dc_status,
                  aes(x = factor(question, levels = dc_status_levels),
                      y = percent)) +
    labs(
        x = "",
...
ggassessment_hist(swc_status,
                  aes(
                      x = factor(status, swc_status_levels),
                      y = percent
                  )) +
    labs(
        x = "",
...
dc_gender_levels <- c("Female", "Male", "Transgender female",
                      ...
                      "Prefer not to answer", "Not listed (please specify)",
                      "Didn't answer")
...
dc_gender <- dcpredata %>%
    ...
    single_answer_summary(gender_identity, dc_gender_levels)

kable(dc_gender, format = 'markdown', row.names = NA,
...
dc_ethnicity_data <- dcpredata %>%
    filter(country == "United States of America") %>% 
    select(starts_with("ethnic_identity"),
           -ethnic_identity_other) %>%
    filter(has_answers(.))

dc_ethnicity_keys <- dc_ethnicity_data %>%
    get_key_multi_answer

dc_ethnicity <- dc_ethnicity_data %>%
    tally_percent_multi_answer %>%
    inner_join(dc_ethnicity_keys, by = "column_name")
...
swc_gender <- swcpredata %>%
    filter(us_workshop == "Yes") %>% 
    single_answer_summary(gender, swc_gender_levels)

kable(swc_gender, format = 'markdown', row.names = NA,
...
swc_ethnicity <- swcpredata %>%
    filter(us_workshop == "Yes") %>% 
    single_answer_summary(race, swc_ethnicity_levels)


kable(swc_ethnicity, format = 'markdown', row.names = NA,
      col.names = c("Software Carpentry's U.S. Respondents' Racial/Ethnic Identity", "n", "%"))
