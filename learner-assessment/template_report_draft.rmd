...
swcpredata %>%
  ...
  arrange(desc(n)) %>% 
  rename(`Why are you attending? Check all that apply.` = WhyAttending)
kable(WhyAttending, format = 'markdown', row.names = NA, col.names = c ("Why learners attend SWC workshops", "n", "%"))
```
...
kable(Satisfaction, format = 'markdown', row.names = NA, col.names = c ("DC Learners satisfaction with current data management practices", "n", "%"))
...
kable(Programming, format = 'markdown', row.names = NA, col.names = c ("DC Learners current programming usage", "n", "%"))

# How do I get the table above to show in order (least to greatest)?
```
...
kable(Exposure, format = 'markdown', row.names = NA, col.names = c ("How learners find out about DC workshops", "n", "%"))
```

```{r}
# How did you find out about this workshop? (SWC, DC)
# Software Carpentry Responses are in columns 'Exposure-Workshop' through 'Column31'
Exposure <- 
swcpredata %>%
...
kable(Exposure, format = 'markdown', row.names = NA, col.names = c ("How learners find out about SWC workshops", "n", "%"))
```
...
```{r}
# Environment (SWC/DC)
# Software Carpentry: I felt comfortable learning in this workshop environment
...
# Software Carpentry Respondents: Perception of Workshop Instructors and Helpers
...
# Perception of instructors and helpers (SWC/DC)
# Software Carpentry
# Code chunk for likert plot (perception of SWC workshop instructors/helpers)
# ordered_often <-
...
# Software Carpentry
plot(lc_often_inst) +
  ggtitle("Perception of Software Carpentry Workshop Instructors") + 
...
swcpostdata %>% 
  mutate(new_info_fct = fct_relevel(NewInformation, 
                                   ordered_new_information)) %>% 
  tally_and_perc(new_info_fct,
                 na.rm = TRUE)
kable(data_new_information_tally)
```
