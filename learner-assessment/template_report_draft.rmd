...
why_attending_keys <- dcpredata %>%
    select(`WhyAttending`:`Column29`) %>%
    gather() %>%
    filter(!is.na(value)) %>%
    distinct(key, value)

why_attending <- dcpredata %>%
    select(`WhyAttending`:`Column29`) %>%
    filter(rowSums(is.na(.)) < ncol(.)) %>% 
    summarize_all(funs(n = tally_nona, percent = percent_nona)) %>%    
    gather() %>%
    separate(key, c("column", "metric")) %>%
    spread(metric, value) %>%
    inner_join(why_attending_keys, by = c("column" = "key"))

p_work_future <- why_attending %>%
    filter(column == "WhyAttending") %>%
    pull(percent)
...
why_attending %>% 
    ...
    kable(format = 'markdown', row.names = NA,
          col.names = c ("Why learners attend Data Carpentry workshops?",
                         "n", "%"))
```
...
Satisfaction <- dcpredata %>%
    ...
    mutate(CurrentSatisfaction = replace(CurrentSatisfaction, is.na(CurrentSatisfaction), "Didn't answer")) %>% 
    gather(col, Satisfaction) %>%
    tally_and_perc(Satisfaction, na.rm = FALSE) %>% 
    arrange(factor(Satisfaction, levels = satisfaction_levels))
...
ggplot(dcpostdata_workshop_tally, 
  ...
  ylab("% Respondents") +
  ggtitle("Marjority of Data Carpentry Respondents \nAttend Ecology Workshops") +
...
ProgrammingLanguage <- 
    ...
    tally_and_perc(ProgrammingLanguage, 
                   na.rm = TRUE) %>% 
    arrange(factor(ProgrammingLanguage,
                   levels = c("R", "Python", "I don't know/I don't remember",
                              "Neither"))) %>% 
...
## new.
swcpostdata$NewInformation <- iconv(swcpostdata$NewInformation,
                                    "latin1", "ASCII", sub=" ")
swcpostdata$NewInformation <- gsub("\\s+", " ", swcpostdata$NewInformation)

newinfo <- c("None of it", "Some of it",
...
kable(net_promoter_score, format = 'markdown', row.names = NA, col.names = c ("Data Carpentry Promoter Score", "n", "%"))
...
`r percent_promoters`% of Data Carpentry respondants are promoters (i.e. would recommend a workshop). 

```{r}
# Software Carpentry
# Q: I would recommend this workshop to a friend/colleague
# I will only include the promoter score info
...
#                   na.rm = TRUE) %>%
#    arrange(factor(Recommend, levels = recommend)) %>% 
...
net_promoter_score_swc <- 
    ...
    count(category) %>%
    mutate("%" = (n / sum(n))*100)
    kable(net_promoter_score_swc, format = 'markdown', row.names = NA, col.names = c ("Software Carpentry Promoter Score", "n", "%"))
    
    percent_promoters_swc <- net_promoter_score_swc %>%
    filter(category == "Promoter") %>%
    pull(`%`) %>%
    round()
```

For Software Carpentry respondents, `r percent_promoters_swc`% are promoters.
```{r}
# Qualitative Analysis (SWC/DC)
```
