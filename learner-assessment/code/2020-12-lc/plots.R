...
col_names_table <- function() {
  read_csv("../data/2020-12-lc/col-names.csv")
}
...
prepare_plot_multi_data <- function(.data, var) {
  ...
  all_data <- .data %>%
    ...
    tidyr::separate_rows(!!quo_var, sep = "(;\\s?)|(,\\s{2})") %>%
...
plot_group <- function(.data, prefix, y_levels, title) {
  ...
  plot_data %>%
    ...
    add_percent() %>%
    ggassessment_lolli(
      aes(y = factor(value, levels = y_levels),
        x = percent),
      color = "cp",
      nudge_x = .02
    ) +
    facet_wrap(
      ~ key,
      ncol = 2,
      labeller = as_labeller(lbler),
      scales = "free"
    ) +
    scale_y_discrete(labels = str_wrap(y_levels, 13)) +
    labs(
      title = str_wrap(title, 58)
    )
}
...
pre_post_labels <- function(pre_post) {

  pre_post %>%
    mutate(diff = round(post - pre, 1),
      diff = case_when(
        diff > 0  ~ paste0("+", diff),
    ...
    left_join(
      tibble(
        skill = c(
          "skill_confidence_programming",
          "skill_data_raw",
          "skill_efficient_programming",
          "skill_overcome_problem",
          "skill_reproducibility_programming",
          "skill_search_answers",
          "skill_write_program"
        ),
        labels = c("Confidence Programming", "Raw Data",
          "Programming is Efficient", "Overcome Problems",
          "Programming for Reproducibility",
          "Search for Answers", "Write Programs"),
        long_labels = c(
    ...
    mutate(
      labels_with_diff = paste0(labels, " (", diff, ")"),
      pre_post = "post"
    )
}
