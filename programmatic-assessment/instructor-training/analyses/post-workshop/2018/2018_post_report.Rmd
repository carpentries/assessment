...
### When are respondents completing the post instructor training survey?
...
kable(postdata_pace_tally, format = 'markdown', col.names = c("Workshop Pace", "n", "%"))
```
...
ggplot(postdata_handson_tally,
  ...
  ylab("% Respondents") +
  ggtitle("Trainees Perception of Workshop Balance: \nLecture vs. Hands-On") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic(base_size = 14)
```
...
ability_before_plot <- 
   ggplot(before_ability, 
    ...
    theme(legend.position = "bottom", 
          legend.title=element_blank()) + 
 guides(fill = guide_legend(nrow = 3)) # wraps legend
```
...
Trainees were asked how likely they are to recommend a Software and/or Data Carpentry workshop to a friend or colleague using the [Net Promoter Score](https://en.wikipedia.org/wiki/Net_Promoter). The scoring for this question based on a 0 to 10 scale. Respondents scoring from 0 to 6 are labeled *Detractors*, and are believed to be less likely to recommend a workshop. Those who respond with a score of 9 to 10 are called *Promoters*, and are considered likely to recommend a workshop. Respondents between 7 and 8 are labeled *Passives*, and their behavior falls in the middle of Promoters and Detractors.
